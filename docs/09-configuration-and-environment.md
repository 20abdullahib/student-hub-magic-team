...
6. Known quirks (do not silently change): typo folders (`login-singup`, `genration`, `handel-search-requset.js`), `Auth::admin()` in AdminMiddleware, dead `AboutController::getSuggestions()` with `dd()`, and **`DropboxService` calls with `Http::withoutVerifying()`** (verifyCredentials, refreshAccessToken, validateToken — SSL verification is intentionally disabled on all Dropbox token/auth calls). Related gotcha: **Dropbox RPC endpoints reject `post($url, [])`** (Laravel sends an empty JSON *array*); always send an explicit `'{}'` body (`withBody('{}', 'application/json')`) for bodyless RPC calls, and never shrink `dropbox_accounts.access_token` below TEXT (Dropbox `sl.u.` tokens are ~1500–2000 chars).
